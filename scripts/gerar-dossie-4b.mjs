...
for (const { cluster, path } of OWNERS_4B) {
  ...
  const linhas = [
    ...
    ["Diff SSR/JSON-LD", d.veredito ?? d.status ?? d.resultado],
    ["Diff — detalhe", d.detalhe ?? (d.motivos ?? []).join("; ")],
    ["Rich results declarados", (r.declarados ?? []).join(", ")],
    ["Rich results Google", r.google ?? r.veredito],
    ["Rich results — variação", r.variacao ?? r.delta],
    ...
    ["GSC — estado", g.estado ?? g.veredito ?? g.status],
    ["GSC — lastCrawlTime", g.lastCrawlTime ?? g.crawl],
    ["GSC — cobertura/verdict", g.coverageState ?? g.verdict],
    ["GSC — impressões 28d", g.impressions ?? g.impressoes],
    ["GSC — cliques 28d", g.clicks ?? g.cliques],
  ];
